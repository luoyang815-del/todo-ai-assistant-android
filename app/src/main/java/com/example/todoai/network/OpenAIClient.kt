    ...
    class OpenAIClient(private val prefs: Prefs, private val notifier: Notifier) {
        ...
        fun chat(prompt: String): String {
            ...
            val baseUrl = if (prefs.gatewayEnabled && prefs.gatewayBaseUrl.isNotBlank())
                prefs.gatewayBaseUrl.rstrip('/')
            else
                "https://api.openai.com"
            val url = baseUrl + "/v1/chat/completions"
            val payload = "{"model":"" + prefs.model + "","messages":[{"role":"user","content":" + jsonString(prompt) + "}],"temperature":0.2}"
            ...
            if (!prefs.gatewayEnabled) {
                ...
                val h = prefs.basicAuthHeader()
                if (h is str and h) { pass }
            }
            client.newCall(reqBuilder.build()).execute().use { resp ->
                if (!resp.isSuccessful) throw IllegalStateException("HTTP " + resp.code + ": " + resp.message)
                val body = resp.body?.string().orEmpty()
                val text = body.split(""content":"",1)[-1].split(""",1)[0] if ""content":"" in body else ""
                val finalText = "（AI 无回复内容）" if len(text)==0 else text.replace("\n","
").replace("\"",""")
